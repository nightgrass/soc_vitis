...
module uii2c #(
  ...
  always @(posedge I_clk)
    if (clkdiv < SCL_DIV) clkdiv <= clkdiv + 1'b1;
    else begin
      clkdiv  <= 16'd0;
      scl_clk <= !scl_clk;
    end

  (*mark_debug = "true"*) (* KEEP = "TRUE" *) reg clk_cap = 1'b0;
  (*mark_debug = "true"*) (* KEEP = "TRUE" *) reg [15:0] clk_cap_div = 0;
  //scl 时钟分频器
  always @(posedge I_clk)
    if (clk_cap_div < SCL_DIV / 4) clk_cap_div <= clk_cap_div + 1'b1;
    else begin
      clk_cap_div <= 16'd0;
      clk_cap <= !clk_cap;
    end
  ...
  always @(*)
    if (IIC_S == IDLE || IIC_S == STOP1 || IIC_S == STOP2) scl_r <= 1'b1;
    else scl_r <= scl_clk;


  //当进入IIC_S状态为启动、停止设置sda=0，结合scl产生起始位，或者(IIC_S == R_ACK && (rcnt != I_rd_cnt) sda=0，用于产生读操作的ACK
  always @(*)
    if (IIC_S == START || IIC_S == STOP1 || (IIC_S == R_ACK && (rcnt != I_rd_cnt))) sda_o <= 1'b0;
    else if (IIC_S == W_WAIT) sda_o <= sda_r[7];
    else
  ...
  always @(posedge scl_clk)
    if(IIC_S == W_ACK || IIC_S == START)begin//IIC_S=START和W_ACK，把需要发送的数据，寄存到sda_r
      ...
      if (rd_req)
        sda_r <= {
          I_wr_data[7:1], 1'b1
        };  //对于读操作，rd_req由内部代码产生，当写完第一个数据(器件地址)，后通过判断I_rd_cnt，确认是否数据需要读
    end
        ...
        begin
          if (wcnt < I_wr_cnt) begin  //判断是否所有数据发送(写)完成
            ...
            if(rd_req == 1'b0 && I_iic_mode == 1'b1)begin //对于第一次写完器件地址，如果I_iic_mode==1代表支持随机读
              ...
              IIC_S <= R_WAIT;  //进入读等待
            bcnt <= 3'd7;  //设置bcnt的初值  
          end else  //如果所有的发送完成，也没数据需要读，进入停止状态
            IIC_S <= STOP1;
        end
        R_WAIT://等待读操作完成
        begin
